...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **CRM** | Kanban premium de leads com atividades e histórico | Kanban funcional com policies ativas e service layer | Parcial em PRD de CRM | `agency.$slug.crm.tsx`, `crm.ts` | Existentes, mas em formato lista simples | `leads`, `lead_stages`, `lead_activities` | Existentes | `deleted_at`, `activities` | `leads` sem `deleted_at` nativo inicialmente | **Sim** (Resolvido em migration emergencial) | Parcial | **Não** (RLS ativa sem policy no início) | Não aplicável | Não | Não | Parcial | Alto | Refatorar para Kanban real + blindagem de policies |
| **Embarques** | Kanban de embarques com alertas e contagem | Kanban operacional sem hardcode e type safe | Parcial em PRD | `agency.$slug.boarding.tsx`, `boarding.ts` | Existente | `boarding_cards` | Existente | `departure_date`, `passengers_count` | Ausentes inicialmente no banco real | **Sim** (Corrigido com SQL manual posterior) | Parcial | Sim | Não aplicável | Não | Não | Mismatch | Alto | Garantir migrations e tipos unificados |
| **Admin Master** | Painel global master de planos, políticas e auditoria | Interface robusta integrada com dados reais do SaaS | Parcial em PRD | `admin.*.tsx`, `admin.ts` | Existente (Visual básico) | `plans`, `policy_documents`, `global_settings` | Existentes (vazias) | Leitura de tabelas reais de planos e termos | Dados vindos de JSON mockado em `api_keys` | **Sim** (Dados reais no frontend não existem) | Não | Sim | Não aplicável | Parcial | Não | Mismatch | Alto | Migrar dados de `api_keys` para tabelas dedicadas |
| **Contratos** | Assinatura de contrato ICP com registro de IP/UA | Registro WORM com hash SHA-256 e PDF em storage | Completo em audit de contrato | `m.contract.$token.tsx`, `payment.ts` | Existente | `contracts` | Existente | `ip`, `user_agent`, `signed_hash`, `pdf_url` | `pdf_url` contendo Base64 em vez de path | **Sim** (Base64 gravado diretamente no banco) | Sim | Sim | `contract-pdfs` bucket desprotegido | Sim | Não | Parcial | Médio | Atualizar RPC para salvar PDF no Storage e hash no servidor |
| **CMS Builder** | Builder avançado com versionamento e preview | Editor de blocos drag & drop com rascunhos | Parcial em CMS rules | `agency.$slug.portal.pages.tsx` | Existente (Básico em Dialog) | `portal_pages` | Existente | Versionamento de blocos e preview isolado | Blocos puros em JSON sem draft/publish | **Sim** (Não há versionamento ou drafts reais) | Parcial | Sim | Não aplicável | Não | Não | Raso | Alto | Implementar tabela de versões e sanitização HTML |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **CRM** | Arquivar Lead | `LeadRow.tsx` | `crm.ts` | `updateLead` | Tabela `leads` | `deleted_at` | Sim (adicionado recentemente) | Sim (sem policies inicialmente) | Não | Não | Sim | Mismatch | Implementar policies e renderizar ErrorState se falhar |
| **Embarques** | Salvar Cartão | `BoardingForm.tsx` | `boarding.ts` | `saveBoardingCard` | Tabela `boarding_cards` | `departure_date`, `passengers_count` | Sim (via fix manual) | Sim | Não | Não | Sim | Mismatch | Garantir compilação com tipos gerados sem `as any` |
| **Admin Agentes** | Listar Agentes | `admin.agents.tsx` | Inline `supabase` | `select` | View `vw_admin_agents` | Multi-campos | Sim (via fix manual) | Não | Não | Não | Sim | Mismatch | Transformar view em migration oficial e tipar no TS |
| **Configuração Brand** | Salvar Brand | `BrandForm.tsx` | `settings.ts` | `updateBrand` | Tabela `api_keys` (JSON) | `api_keys.metadata` | Sim (como JSON) | Sim | Não | Não | Sim | Mismatch | Migrar para tabela `global_settings` dedicada |
