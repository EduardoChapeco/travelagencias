...
if (!SOURCE_URL || !SOURCE_KEY || !DEST_URL || !DEST_KEY) {
  ...
  console.error(
    "Execute com: SOURCE_SUPABASE_URL=... SOURCE_SERVICE_ROLE_KEY=... DEST_SUPABASE_URL=... DEST_SERVICE_ROLE_KEY=... node scripts/migrate-auth.mjs",
  );
  process.exit(1);
}
...
async function run() {
  ...
  const { data: usersData, error: listError } = await sourceSb.auth.admin.listUsers({
    perPage: 1000,
  });
  ...
  console.log(
    "ATENÇÃO: Instrua seus usuários a utilizar a opção 'Esqueci minha senha' no primeiro acesso, pois as senhas antigas não puderam ser copiadas.",
  );
}
