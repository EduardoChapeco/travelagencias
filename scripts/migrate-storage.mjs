...
if (!SOURCE_URL || !SOURCE_KEY || !DEST_URL || !DEST_KEY) {
  ...
  console.error(
    "Execute com: SOURCE_SUPABASE_URL=... SOURCE_SERVICE_ROLE_KEY=... DEST_SUPABASE_URL=... DEST_SERVICE_ROLE_KEY=... node scripts/migrate-storage.mjs",
  );
  process.exit(1);
}
...
async function migrateBucket(bucket, prefix = "") {
  ...
  for (const item of data) {
    ...
    // Download da Origem
    const { data: fileBlob, error: downloadError } = await sourceSb.storage
      .from(bucket)
      .download(fullPath);
    if (downloadError) {
    ...
    // Upload pro Destino
    const { data: uploadData, error: uploadError } = await destSb.storage
      .from(bucket)
      .upload(fullPath, fileBuf, {
