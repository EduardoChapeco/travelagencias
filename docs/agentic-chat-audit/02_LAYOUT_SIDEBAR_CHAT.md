...
- **Tokens**: `--accent` e `--accent-foreground` estão declarados no arquivo [styles.css](file:///c:/Users/Turis Tecnologia SMO/.gemini/antigravity-ide/scratch/travelagencias/src/styles.css):
...
- **Componente**: Em [SlimSidebar.tsx](file:///c:/Users/Turis Tecnologia SMO/.gemini/antigravity-ide/scratch/travelagencias/src/components/shell/SlimSidebar.tsx), a classe `bg-accent text-accent-foreground font-semibold` é aplicada dinamicamente se o item estiver ativo.
...
Os estados de colapso de layout estão implementados em [SlimSidebar.tsx](file:///c:/Users/Turis Tecnologia SMO/.gemini/antigravity-ide/scratch/travelagencias/src/components/shell/SlimSidebar.tsx):
...
- **Persistência das Preferências**: Gravado e lido no `localStorage` via chaves `turis.sidebar.nav.collapsed` e `turis.sidebar.chat.collapsed`. Mudanças de rota retêm os estados.
