...
* **Teste Executado**: Criação de Cotação VibeTour manual e por IA.
  - *Resultado*: **APROVADO**. Os registros são inseridos com sucesso nas tabelas `quote_requests`, `quote_travelers` e `quote_preferences` na base remota Supabase.
* **Teste Executado**: Indexação de Documentos de Conhecimento e Busca Semântica RAG.
  - *Resultado*: **FALHOU**. Como as tabelas RAG (`knowledge_sources`, `knowledge_documents`, `knowledge_chunks`, `knowledge_embeddings`) e o RPC vetorial `match_knowledge_embeddings` não foram criados no banco de dados de produção (migration pendente), a chamada gera exceções e erros HTTP 400 no painel de controle do agente.
* **Teste Executado**: Isolamento de Tenant (RLS Multi-Tenant).
  - *Resultado*: **FALHOU**. Agentes comuns conseguem alterar perfis de scoring e regras de decisão globais porque a migration de segurança `20260727000000_vibetour_global_rules_security.sql` não está ativa no remote.
