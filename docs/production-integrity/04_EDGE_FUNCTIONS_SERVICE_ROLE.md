...
2. **Ausência de Controle de Tenant (Membership):**
   - _Problema:_ O parâmetro `agency_id` é aceito livremente no JSON body enviado pelo frontend. O script da Edge Function utiliza o `supabaseAdmin` para invocar a procedure `pick_active_api_key` ou selecionar diretamente a tabela `api_keys` filtrando pelo `agency_id` recebido.
   - _Impacto:_ Como o cliente administrador ignora RLS, o banco de dados retorna a chave de API da agência sem validar se o usuário do JWT pertence àquela agência. Um usuário autenticado pode consumir tokens e cotas de IA de qualquer agência concorrente cadastrada no sistema apenas alterando o ID no corpo HTTP.
