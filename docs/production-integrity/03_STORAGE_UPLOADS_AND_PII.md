...
1. **Estrutura dos Caminhos (Paths):**
   ...
   * *Falha:* O caminho no checkout público não está vinculado a um identificador seguro da transação ou inscrição (`booking_id`), pois o upload ocorre *antes* de a inscrição ser gravada. No portal de pagamento, todos os arquivos de recibos são salvos na pasta raiz `receipts/` em uma lista plana, sem separação por agência.
