...
* **Tratamento de Echoes:** Quando a coexistência está ativa, o operador pode enviar mensagens pelo próprio celular. O webhook do WhatsApp envia esse evento ao Turis como um "echo" (mensagem enviada pelo próprio número, mas fora da plataforma).
  * O webhook do Turis deve processar e salvar esses echoes com `direction = 'outbound'` e `source = 'whatsapp_business_app'` para que a conversa seja mantida 100% atualizada na tela do Inbox do agente.
