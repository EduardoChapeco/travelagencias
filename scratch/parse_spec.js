...
for (const p of paths) {
  ...
  for (const m of methods) {
    ...
    endpointList.push({
      ...
      responseBodyRef: op.responses?.['200']?.content?.['application/json']?.schema?.$ref || op.responses?.['201']?.content?.['application/json']?.schema?.$ref || ''
    });
  }
}
...
fs.writeFileSync('docs/infotravel-audit/reference/structured_inventory.json', JSON.stringify(endpointList, null, 2), 'utf8');
