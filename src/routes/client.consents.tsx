...
function ClientConsentsPage() {
  ...
  return (
    <>
      <PageHeader title="Privacidade e Termos" description="Gerencie seus consentimentos e termos legais vigentes." />
      ...
      {q.data?.missing.length === 0 && (
         <div className="mb-8 rounded-lg border border-border bg-success-bg/20 p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Você não possui nenhum termo obrigatório pendente de aceite.</p>
         </div>
      )}
      ...
      {q.data && q.data.missing.length > 0 && (
         <div className="mb-8 space-y-3">
            {q.data.missing.map(m => (
               <div key={m.id} className="rounded-xl border border-warning/50 bg-warning-bg/10 p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex gap-3">
                     <FileText className="h-5 w-5 text-warning shrink-0" />
                     <div>
                        <h4 className="text-sm font-bold text-foreground">{m.title} <span className="text-xs font-normal text-muted-foreground ml-1">v{m.version}</span></h4>
                        <p className="text-xs text-muted-foreground mt-1 max-w-lg">
                           Para continuar utilizando os serviços da agência, é necessário revisar e aceitar a versão mais recente deste documento.
                        </p>
                        {m.url && (
                           <a href={m.url} target="_blank" rel="noreferrer" className="text-xs text-primary hover:underline mt-2 inline-block font-medium">Ler documento completo</a>
                        )}
                     </div>
                  </div>
                  <PrimaryButton 
      ...
      <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Histórico de Aceites</h3>
      {q.data?.acceptances.length === 0 && <EmptyState title="Sem aceites" description="Nenhum registro de aceite encontrado." />}
      <div className="space-y-2">
        {q.data?.acceptances.map((a) => (
          <div key={a.id} className="flex flex-col sm:flex-row sm:items-center justify-between rounded-lg border border-border bg-surface p-4 gap-4">
            <div className="flex items-start gap-3">
              <CheckCircle2 className="h-5 w-5 text-success mt-0.5" />
              <div>
                <div className="text-sm font-medium">{a.policy_documents?.title} <span className="text-xs text-muted-foreground ml-1">v{a.policy_documents?.version}</span></div>
                <div className="text-xs text-muted-foreground mt-1 flex items-center gap-2">
                   <Clock className="h-3 w-3" /> {fmtDate(a.accepted_at)}
                   {a.ip_address && <span className="text-[10px] font-mono bg-surface-alt px-1.5 py-0.5 rounded border border-border">IP: {a.ip_address}</span>}
                </div>
              </div>
            </div>
            <div>
