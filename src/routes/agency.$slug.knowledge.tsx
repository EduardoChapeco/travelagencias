    ...
    <>
          ...
          <button key={a.id} onClick={() => setViewing(a)} className="rounded-lg border border-border bg-surface p-4 text-left hover:border-border-strong">
            ...
            <div className="mt-2 font-semibold flex items-center justify-between gap-2">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={(e) => { e.stopPropagation(); setEditing(a); }} className="p-1 hover:bg-surface-alt rounded text-muted-foreground"><Edit2 className="w-3.5 h-3.5"/></button>
              </div>
            </div>
            <div className="text-xs text-muted-foreground flex items-center justify-between">
              {a.category ?? "Geral"}
              {!a.is_internal && <span className="flex items-center gap-1"><Eye className="w-3 h-3" /> {a.views ?? 0}</span>}
            </div>
            {a.tags?.length > 0 && (
      ...
      {(open || editing) && agency && (
        <ArticleSheet 
          ...
          onSaved={() => { setOpen(false); setEditing(null); qc.invalidateQueries({ queryKey: ["knowledge", agency.id] }); }} 
        />
      )}
    </>
  );
...
function ArticleSheet({ agencyId, agencySlug, initialData, onClose, onSaved }: { agencyId: string; agencySlug: string; initialData: any; onClose: () => void; onSaved: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = {
      agency_id: agencyId, title, slug: finalSlug, category: category || null,
      ...
      <div className="flex h-full w-full max-w-2xl flex-col border-l border-border bg-surface" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            {previewUrl && (
              <a href={previewUrl} target="_blank" rel="noreferrer" className="flex h-8 items-center gap-1.5 rounded-md border border-border px-2.5 text-xs font-medium hover:bg-surface-alt">
                Ver publicado
              </a>
            )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <form id="kb-form" onSubmit={submit} className="space-y-4">
            ...
            <Field label="Tags (separadas por vírgula)"><Input value={tags} onChange={(e) => setTags(e.target.value)} /></Field>
            ...
            <div className="pt-4 border-t border-border mt-4">
              <label className="flex items-center gap-2 text-sm font-semibold cursor-pointer">
                <input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} className="w-4 h-4 rounded border-input" /> 
                Artigo apenas para uso interno (Agência)
              </label>
              <p className="text-[11px] text-muted-foreground mt-1 ml-6">
                Se desmarcado, o artigo ficará disponível publicamente no portal do cliente (Base de Conhecimento).
              </p>
            </div>
          </form>
        </div>
