...
function SupportPage() {
  ...
  return (
    <>
              ...
              {q.data.data.map((t) => (
                <tr key={t.id} className="border-t border-border hover:bg-surface-alt/30">
                  ...
                  <td className="px-3 py-2.5">
                    <Link to="/agency/$slug/support/$ticket_id" params={{ slug, ticket_id: t.id }} className="font-medium hover:underline">
                      {t.title}
                    </Link>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{t.type}</td>
                  <td className="px-3 py-2.5">
                    <StatusBadge tone={t.priority === "high" || t.priority === "urgent" ? "danger" : t.priority === "low" ? "neutral" : "warning"}>{t.priority}</StatusBadge>
                  </td>
                  <td className="px-3 py-2.5">
                    <StatusBadge tone={t.status === "resolved" ? "success" : t.status === "in_progress" ? "info" : "warning"}>{t.status}</StatusBadge>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{fmtDate(t.sla_deadline)}</td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{fmtDate(t.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {open && agency && (
        <NewTicket agencyId={agency.id} onClose={() => setOpen(false)} onCreated={() => { setOpen(false); qc.invalidateQueries({ queryKey: ["tickets", agency.id] }); }} />
      )}
    </>
  );
}

function NewTicket({ agencyId, onClose, onCreated }: { agencyId: string; onClose: () => void; onCreated: () => void }) {
