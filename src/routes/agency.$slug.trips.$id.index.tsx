...
function TripOverview() {
  ...
  if (tripQ.isLoading || !form) return <div className="text-sm text-muted-foreground p-6">Carregando…</div>;
  if (!tripQ.data) return <div className="text-sm text-muted-foreground p-6">Viagem não encontrada.</div>;
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_280px]">
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          save.mutate({
            title: t.title, destination: t.destination, travel_start: t.travel_start, travel_end: t.travel_end,
            status: t.status, currency: t.currency, total_sale: t.total_sale, total_cost: t.total_cost,
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Título (Uso Interno)"><Input value={t.title} onChange={(e) => setForm({ ...t, title: e.target.value })} className="bg-surface-alt" /></Field>
          <Field label="Código Interno (Opcional)"><Input value={t.code ?? ""} onChange={(e) => setForm({ ...t, code: e.target.value })} className="bg-surface-alt" /></Field>
        </div>
        <Field label="Destino Principal"><Input value={t.destination ?? ""} onChange={(e) => setForm({ ...t, destination: e.target.value })} className="bg-surface-alt" /></Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Data de Ida"><Input type="date" value={t.travel_start ?? ""} onChange={(e) => setForm({ ...t, travel_start: e.target.value || null })} className="bg-surface-alt" /></Field>
          <Field label="Data de Volta"><Input type="date" value={t.travel_end ?? ""} onChange={(e) => setForm({ ...t, travel_end: e.target.value || null })} className="bg-surface-alt" /></Field>
        </div>
        <div className="grid grid-cols-3 gap-4 border-t border-border pt-4 mt-2">
          <Field label="Status Operacional">
            <Select value={t.status} onChange={(e) => setForm({ ...t, status: e.target.value })} className="bg-surface-alt">
          ...
          <Field label="Venda (R$)"><Input type="number" min={0} step="0.01" value={t.total_sale} onChange={(e) => setForm({ ...t, total_sale: +e.target.value || 0 })} className="bg-surface-alt" /></Field>
          <Field label="Custo Líquido (R$)"><Input type="number" min={0} step="0.01" value={t.total_cost} onChange={(e) => setForm({ ...t, total_cost: +e.target.value || 0 })} className="bg-surface-alt" /></Field>
        </div>
        <Field label="Anotações Internas (Não visíveis ao cliente)"><Textarea value={t.notes ?? ""} onChange={(e) => setForm({ ...t, notes: e.target.value })} className="bg-surface-alt min-h-[100px]" /></Field>
        <div className="pt-2 flex justify-end">
          <PrimaryButton type="submit" disabled={save.isPending}>
            {save.isPending ? "Salvando…" : <><Save className="mr-2 h-4 w-4" /> Salvar Alterações</>}
          </PrimaryButton>
        </div>
      </form>
