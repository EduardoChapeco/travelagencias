...
function HomePage() {
  ...
  const q = useQuery({
    queryKey: ["portal-home", agency_slug],
    queryFn: async () => {
      ...
      const [company, tours, posts, homePage] = await Promise.all([
        ...
        supabase.from("portal_pages").select("blocks, seo").eq("agency_id", agency.id).eq("slug", "home").eq("is_published", true).maybeSingle(),
      ]);
  ...
  const { agency, company, tours, posts, homePage } = q.data;
    ...
    <>
      <header className="relative px-6 py-24 text-center bg-surface overflow-hidden border-b border-border/50">
        {company?.cover_image_url ? (
          <>
            <img src={company.cover_image_url} alt="Cover" className="absolute inset-0 h-full w-full object-cover opacity-20" />
        ...
        <div className="relative z-10 flex flex-col items-center justify-center">
          {agency.logo_url && <img src={agency.logo_url} alt={agency.name} className="mb-6 h-28 w-28 rounded-2xl object-cover ring-4 ring-surface shadow-xl" />}
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">{agency.name}</h1>
          {company?.short_description && <p className="mx-auto mt-4 max-w-xl text-lg text-muted-foreground font-medium">{company.short_description}</p>}
          
          <div className="mt-6 flex items-center justify-center gap-4">
            {company?.instagram && <a href={`https://instagram.com/${company.instagram.replace("@", "")}`} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-brand"><Instagram className="h-5 w-5" /></a>}
            {company?.facebook && <a href={company.facebook} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-brand"><Facebook className="h-5 w-5" /></a>}
            {company?.youtube && <a href={company.youtube} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-brand"><Youtube className="h-5 w-5" /></a>}
            {company?.linkedin && <a href={company.linkedin} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-brand"><Linkedin className="h-5 w-5" /></a>}
          </div>
        </div>
      </header>
        ...
        {tours.length > 0 && (
          <section className="mx-auto max-w-5xl">
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3">
              {tours.map((t) => (
                <Link key={t.id} to="/p/$agency_slug/tour/$id" params={{ agency_slug, id: t.id }} className="group flex flex-col overflow-hidden rounded-2xl border-2 border-border/50 bg-surface transition-all hover:-translate-y-1 hover:border-brand/40">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {company.gallery.map((img: string, i: number) => (
                <div key={i} className="aspect-square overflow-hidden rounded-xl bg-surface-alt">
                  <img src={img} alt={`Gallery ${i}`} className="h-full w-full object-cover transition-transform hover:scale-105" />
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mx-auto max-w-5xl">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-8">
                <h2 className="mb-6 text-xl font-bold tracking-tight flex items-center gap-2"><MapPin className="h-5 w-5 text-brand" /> Contato e Localização</h2>
                <div className="space-y-4 text-sm font-medium">
                  {company.email && <div><span className="text-muted-foreground block text-xs uppercase tracking-wider mb-0.5">Email</span><a href={`mailto:${company.email}`} className="hover:text-brand transition-colors">{company.email}</a></div>}
                  {company.phone && <div><span className="text-muted-foreground block text-xs uppercase tracking-wider mb-0.5">Telefone</span>{company.phone}</div>}
                  {company.whatsapp && <div><span className="text-muted-foreground block text-xs uppercase tracking-wider mb-0.5">WhatsApp</span><a href={`https://wa.me/${company.whatsapp.replace(/\D/g, "")}`} className="hover:text-brand transition-colors">{company.whatsapp}</a></div>}
                  {company.website && <div><span className="text-muted-foreground block text-xs uppercase tracking-wider mb-0.5">Website</span><a href={company.website} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-brand transition-colors">{company.website} <ExternalLink className="h-3 w-3" /></a></div>}
                  {company.address?.street && (
                    <div className="pt-2">
                      <span className="text-muted-foreground block text-xs uppercase tracking-wider mb-0.5">Endereço</span>
                      {company.address.street}, {company.address.number}
                      {company.address.complement && ` - ${company.address.complement}`}
                      <br />{company.address.neighborhood} - {company.address.city}/{company.address.state}
                    </div>
                  )}
                </div>
              </div>
              ...
              {company.business_hours && (
                <div className="rounded-2xl border border-border bg-surface p-8">
                  <h2 className="mb-6 text-xl font-bold tracking-tight flex items-center gap-2"><Clock className="h-5 w-5 text-brand" /> Horários de Atendimento</h2>
                  <div className="space-y-3 text-sm">
                    {["seg", "ter", "qua", "qui", "sex", "sab", "dom"].map((day) => {
                      ...
                      return (
                        <div key={day} className="flex justify-between items-center border-b border-border/40 pb-2 last:border-0 last:pb-0">
                          ...
                          {h.closed ? (
                            ...
                            <span className="font-mono text-foreground font-semibold">{h.open} - {h.close}</span>
                          )}
                        </div>
                      );
                    })}
