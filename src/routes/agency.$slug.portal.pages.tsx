  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <button onClick={() => setEditingPage("new")} className="flex h-9 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-semibold text-primary-foreground">
      ...
      {q.data?.length === 0 && <EmptyState title="Sem páginas" description="Crie páginas institucionais para o seu portal." />}
      ...
      {q.data && q.data.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {q.data.map((p) => (
                <tr key={p.id} className="border-t border-border hover:bg-surface-alt/30 transition-colors">
                  <td className="px-3 py-2.5 font-medium cursor-pointer" onClick={() => setEditingPage(p)}>{p.title}</td>
                  <td className="px-3 py-2.5 font-mono text-xs text-muted-foreground">/{p.slug}</td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground capitalize">{p.template ?? "padrão"}</td>
                  ...
                  <td className="px-3 py-2.5 text-right flex justify-end items-center gap-2">
                    ...
                    <button onClick={() => togglePublish(p)} className="text-xs text-primary hover:underline">
                      {p.is_published ? "Despublicar" : "Publicar"}
                    </button>
                    <button onClick={() => deletePage(p)} className="text-muted-foreground hover:text-destructive">
...
function PageEditorSheet({
  ...
  const addBlock = (type: PortalBlock["type"]) => {
    ...
    switch (type) {
      case "hero": newBlock = { id, type: "hero", title: "", subtitle: "", bg_image_url: "", cta_label: "", cta_link: "" }; break;
      case "text": newBlock = { id, type: "text", content: "", align: "left", image_url: "" }; break;
      case "gallery": newBlock = { id, type: "gallery", images: [] }; break;
      case "contact": newBlock = { id, type: "contact", title: "Fale conosco", text: "" }; break;
    }
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-overlay" onClick={onClose}>
      <div className="flex h-full w-full max-w-2xl flex-col border-l border-border bg-surface" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button type="button" onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-md border border-border hover:bg-surface-alt">
            <X className="h-4 w-4" />
          </button>
        </div>

        <div className="flex border-b border-border px-6">
          <button onClick={() => setTab("content")} className={`py-3 px-2 text-xs font-medium border-b-2 ${tab === "content" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>Conteúdo & Blocos</button>
          <button onClick={() => setTab("seo")} className={`py-3 px-2 text-xs font-medium border-b-2 ${tab === "seo" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>SEO</button>
        </div>
            ...
            {tab === "content" && (
              <>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Título interno *"><Input required value={title} onChange={(e) => setTitle(e.target.value)} /></Field>
                  <Field label="URL (Slug)"><Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder={slugify(title)} /></Field>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-border">
                  ...
                  {blocks.length === 0 && (
                    <div className="rounded-lg border border-dashed border-border p-8 text-center">
                      <p className="text-sm text-muted-foreground">Esta página ainda não tem blocos de conteúdo.</p>
                      <p className="text-xs text-muted-foreground mt-1">Adicione um bloco acima para começar.</p>
                    </div>
                  )}

                  <div className="space-y-4">
                    {blocks.map((block, index) => (
                      <div key={block.id} className="rounded-lg border border-border bg-surface-alt/20 shadow-sm overflow-hidden">
                        <div className="flex items-center justify-between bg-surface-alt/40 px-3 py-2 border-b border-border">
                          ...
                          <div className="flex items-center gap-1">
                            <button type="button" onClick={() => moveBlock(index, -1)} disabled={index === 0} className="p-1 text-muted-foreground hover:text-foreground disabled:opacity-30">↑</button>
                            <button type="button" onClick={() => moveBlock(index, 1)} disabled={index === blocks.length - 1} className="p-1 text-muted-foreground hover:text-foreground disabled:opacity-30">↓</button>
                            <button type="button" onClick={() => removeBlock(block.id)} className="p-1 ml-2 text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                          </div>
                        </div>
                        ...
                        <div className="p-4 space-y-4">
                          {block.type === "hero" && (
                            <>
                              <Field label="Título (Headline)"><Input value={block.title} onChange={e => updateBlock(block.id, { title: e.target.value })} /></Field>
                              <Field label="Subtítulo"><Input value={block.subtitle} onChange={e => updateBlock(block.id, { subtitle: e.target.value })} /></Field>
                              <div className="grid grid-cols-2 gap-3">
                                <Field label="Botão (Label)"><Input value={block.cta_label} onChange={e => updateBlock(block.id, { cta_label: e.target.value })} placeholder="Ex: Ver pacotes" /></Field>
                                <Field label="Botão (Link)"><Input value={block.cta_link} onChange={e => updateBlock(block.id, { cta_link: e.target.value })} placeholder="/pacotes" /></Field>
                              </div>
                              <FileUploader
                          ...
                          {block.type === "text" && (
                            <>
                              <Field label="Conteúdo (Markdown)">
                                <Textarea value={block.content} onChange={e => updateBlock(block.id, { content: e.target.value })} rows={5} />
                              </Field>
                          ...
                          {block.type === "contact" && (
                            <>
                              <Field label="Título da seção"><Input value={block.title} onChange={e => updateBlock(block.id, { title: e.target.value })} /></Field>
                              <Field label="Texto de introdução"><Textarea value={block.text} onChange={e => updateBlock(block.id, { text: e.target.value })} rows={2} /></Field>
                              <p className="text-[10px] text-muted-foreground mt-1">Este bloco puxará automaticamente as redes sociais e o formulário de lead da agência no portal público.</p>
                            </>
                          )}
                        </div>
                      </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border bg-surface px-6 py-4">
          ...
          <PrimaryButton type="submit" form="page-form" disabled={submitting}>
