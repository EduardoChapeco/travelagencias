...
function Page() {
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("trips").select("id, code, title, status, destination, travel_start, total_sale, currency, agency_id").order("created_at", { ascending: false }).limit(200);
      ...
      {q.data && q.data.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-border bg-surface">
          <table className="w-full text-sm">
            <thead className="bg-surface-alt text-xs text-muted-foreground"><tr><th className="px-3 py-2 text-left">Código</th><th className="px-3 py-2 text-left">Título</th><th className="px-3 py-2 text-left">Agência</th><th className="px-3 py-2 text-left">Status</th><th className="px-3 py-2 text-left">Início</th><th className="px-3 py-2 text-right">Valor</th></tr></thead>
            <tbody>
              {q.data.map((t) => (
                <tr key={t.id} className="border-t border-border">
                  <td className="px-3 py-2.5 font-mono text-xs">{t.code ?? `#${t.id.slice(0, 6)}`}</td>
                  <td className="px-3 py-2.5">{t.title}<div className="text-xs text-muted-foreground">{t.destination ?? ""}</div></td>
                  <td className="px-3 py-2.5 text-xs">{t.agency_name}</td>
                  <td className="px-3 py-2.5"><StatusBadge>{t.status}</StatusBadge></td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{fmtDate(t.travel_start)}</td>
                  <td className="px-3 py-2.5 text-right text-xs">{money(Number(t.total_sale), t.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
