...
function TripBoardingPage() {
  ...
  const saveCheckinLinkMut = useMutation({
    mutationFn: async ({ segmentId, provider, rawUrl }: { segmentId: string; provider: string; rawUrl: string }) => {
      if (!agency) return;
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Monitore cartões de embarque, configure overrides de check-in e visualize a timeline de eventos dos passageiros.
            </p>
          </div>
        </div>
        ...
        <div className="flex bg-surface-alt p-0.5 rounded-lg border border-border/60 text-[11px] font-semibold">
          <button
            ...
            className={cn(
              ...
              activeTab === "cards" ? "bg-surface shadow text-foreground" : "text-muted-foreground hover:text-foreground"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "px-3 py-1.5 rounded-md transition-all flex items-center gap-1.5",
              activeTab === "settings" ? "bg-surface shadow text-foreground" : "text-muted-foreground hover:text-foreground"
            )}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-surface rounded-xl border border-border p-5 space-y-4 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Substitua os deep links de check-in padrão por URLs customizadas para trechos específicos desta viagem.
            </p>

            {isLoadingSegments ? (
              <div className="text-xs text-muted-foreground animate-pulse py-4">Carregando trechos...</div>
            ) : segments.length === 0 ? (
                  ...
                  <div key={seg.id} className={cn("space-y-2", idx > 0 && "pt-4")}>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => setOverrideUrls({ ...overrideUrls, [seg.id]: e.target.value })}
                      ...
                      />
                      <button
                        onClick={() => saveCheckinLinkMut.mutate({
                          segmentId: seg.id,
          ...
          <div className="bg-surface rounded-xl border border-border p-5 space-y-4 shadow-sm">
            ...
            {isLoadingEvents ? (
              <div className="text-xs text-muted-foreground animate-pulse py-4">Carregando timeline...</div>
              ...
              <div className="relative pl-4 border-l border-border/80 space-y-5">
                {boardingEvents.map((evt) => {
                  ...
                  const segmentDesc = seg ? `${seg.airline_code} ${seg.flight_number} (${seg.origin_iata} → ${seg.destination_iata})` : "";
                  ...
                  if (evt.event_type === "checkin_link_clicked") {
                    ...
                    icon = <ExternalLink className="h-3 w-3 text-emerald-600 dark:text-emerald-400" />;
                    ...
                    const issueType = metadata?.issue_type === "delayed" ? "atraso" : "cancelamento";
                    const comment = metadata?.description ? ` - Obs: "${metadata.description}"` : "";
