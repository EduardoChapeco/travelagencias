    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {q.isLoading ? (
        <div className="animate-pulse space-y-4">
          <div className="h-24 glass-card border-none rounded-[var(--radius-card)] border-none"></div>
          <div className="h-24 glass-card border-none rounded-[var(--radius-card)] border-none"></div>
        </div>
      ) : q.data?.length === 0 ? (
        <div className="rounded-[var(--radius-card)] border border-dashed border-border p-12 text-center text-muted-foreground glass-card border-none/50">
          ...
          {q.data?.map((quote: any) => {
            ...
            return (
              <Link
                ...
                to="/client/quotes/$id"
                params={{ id: quote.id }}
                className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-[var(--radius-card)] border-none glass-card border-none hover:border-primary/50 transition-colors gap-4"
              >
