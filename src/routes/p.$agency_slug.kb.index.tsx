...
function PublicKnowledgeBase() {
  ...
  const byCategory = search ? {} : (articles as any[]).reduce((acc: Record<string, any[]>, article: any) => {
    ...
    <div className="min-h-screen bg-surface">
      ...
      <section className="bg-foreground text-background py-20 px-6 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-4">Como podemos ajudar?</h1>
      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        {qArticles.isLoading && <div className="text-center text-muted-foreground">Buscando artigos...</div>}
        
        {search.trim() ? (
          <div>
            <h2 className="text-xl font-bold mb-6">Resultados para "{search}" ({articles.length})</h2>
            <div className="grid gap-4">
              {(articles as any[]).map((a: any) => (
                <Link key={a.id} to={`/p/${agency_slug}/kb/${a.slug}` as any} className="flex items-center justify-between p-5 rounded-2xl bg-background border border-border hover:border-brand/50 transition-all group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-base group-hover:text-brand transition-colors">{a.title}</h3>
                      {a.category && <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{a.category}</span>}
                    </div>
                  </div>
              ...
              {articles.length === 0 && (
                <div className="text-center py-10 bg-background rounded-2xl border border-border">
                  ...
                  <p className="text-muted-foreground text-sm">Tente usar outros termos para a busca.</p>
                </div>
              )}
            </div>
          </div>
            ...
            {Object.keys(byCategory).map((cat) => (
              <div key={cat} className="space-y-4">
                <h3 className="text-lg font-black tracking-tight border-b border-border pb-2 flex items-center justify-between">
                  {cat}
                  <span className="text-xs font-semibold text-muted-foreground bg-surface-alt px-2 py-0.5 rounded-full">{byCategory[cat].length}</span>
                </h3>
                <div className="flex flex-col gap-2">
                  {byCategory[cat].slice(0, 5).map((a: any) => (
                    <Link key={a.id} to={`/p/${agency_slug}/kb/${a.slug}` as any} className="flex items-center justify-between p-3 rounded-xl hover:bg-background border border-transparent hover:border-border transition-all group">
                       <span className="font-semibold text-sm group-hover:text-brand transition-colors">{a.title}</span>
                       <ChevronRight className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
                    </Link>
                  ))}
                  {byCategory[cat].length > 5 && (
                    <div className="px-3 py-2 text-xs font-bold text-brand cursor-pointer">Ver todos de {cat} &rarr;</div>
                  )}
                </div>
              </div>
            ))}
