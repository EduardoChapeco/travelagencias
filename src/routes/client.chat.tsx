...
function ClientChatRoute() {
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-50 border-none rounded-[var(--radius-card)] overflow-hidden mt-4 shadow-none">
        {/* HEADER */}
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6">
          {loading ? (
            <div className="flex flex-col gap-4 animate-pulse">
              <div className="h-10 w-3/4 bg-zinc-200 rounded-[var(--radius-card)] rounded-tl-sm self-start" />
              <div className="h-10 w-1/2 bg-zinc-300 rounded-[var(--radius-card)] rounded-tr-sm self-end" />
            </div>
          ) : timeline.length === 0 ? (
              ...
              if (event.type === "message") {
                ...
                return (
                  <div key={event.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[85%] rounded-[var(--radius-card)] px-4 py-2 text-sm ${
                        isMe
                          ? "bg-primary text-primary-foreground rounded-tr-sm"
                          : "bg-white border-none text-foreground rounded-tl-sm shadow-none"
                      }`}
                    >
                      {event.data.body}
                      <div
              ...
              if (event.type === "ticket") {
                return (
                  <div key={`ticket-${event.id}`} className="flex justify-center my-4">
                    <div className="bg-zinc-100 border border-zinc-200 rounded-[var(--radius-card)] p-3 flex flex-col items-center max-w-sm text-center shadow-none">
              ...
              if (event.type === "kyc") {
                return (
                  <div key={`kyc-${event.id}`} className="flex justify-center my-4">
                    <div className="bg-green-50 border border-green-200 rounded-[var(--radius-card)] p-3 flex flex-col items-center max-w-sm text-center shadow-none">
              ...
              if (event.type === "proposal") {
                ...
                return (
                  <div key={`prop-${event.id}`} className="flex justify-center my-4">
                    <div className={`border rounded-[var(--radius-card)] p-3 flex flex-col items-center max-w-sm text-center shadow-none ${isAccepted ? 'bg-blue-50 border-blue-200' : 'bg-white border-zinc-200'}`}>
