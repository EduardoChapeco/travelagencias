...
function AcceptInvitePage() {
  ...
  const [invite, setInvite] = useState<{ email: string; role: string; accepted_at: string | null; expires_at: string; agency_id: string } | null>(null);
  ...
  if (!invite) return <div className="p-8 text-sm text-destructive">Convite inválido ou expirado.</div>;
  ...
  return (
    <div className="mx-auto mt-16 max-w-md rounded-lg border border-border bg-surface p-6">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Você foi convidado para entrar como <strong>{invite.role}</strong> usando o email <strong>{invite.email}</strong>.
      </p>

      {invite.accepted_at && <div className="mt-4 rounded bg-success/10 p-3 text-sm">Este convite já foi aceito.</div>}
      {expired && <div className="mt-4 rounded bg-destructive/10 p-3 text-sm">Este convite expirou.</div>}

      {!userEmail && !invite.accepted_at && !expired && (
        <div className="mt-4 space-y-3">
          <div className="rounded bg-warning/10 p-3 text-sm">Entre ou cadastre-se com o email <strong>{invite.email}</strong> para aceitar.</div>
          <GhostButton onClick={() => navigate({ to: "/auth/login" })}>Entrar</GhostButton>
          <PrimaryButton onClick={() => navigate({ to: "/auth/register" })}>Criar conta</PrimaryButton>
        </div>
      )}

      {userEmail && emailMismatch && (
        <div className="mt-4 rounded bg-destructive/10 p-3 text-sm">Você está logado como {userEmail}. Saia e entre com {invite.email}.</div>
      )}

      {userEmail && !emailMismatch && !invite.accepted_at && !expired && (
        <PrimaryButton onClick={accept} disabled={submitting} className="mt-4 w-full">{submitting ? "Aceitando…" : "Aceitar convite"}</PrimaryButton>
      )}
    </div>
  );
}
