...
function CorporateApprovePage() {
  ...
  if (rfpQ.isLoading) return <div className="p-12 text-center text-muted-foreground">Carregando detalhes...</div>;
  
  const rfp = rfpQ.data;
  if (!rfp) return <div className="p-12 text-center text-muted-foreground">Orçamento não encontrado ou token expirado.</div>;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-sans">
      <header className="h-16 border-b border-border bg-surface flex items-center px-6">
        <div className="font-bold tracking-tight text-foreground">{rfp.agency?.name || "TravelOS"} B2B</div>
      </header>
        ...
        <div className="bg-surface border border-border rounded-2xl shadow-xl overflow-hidden">
          <div className="p-8 border-b border-border">
            ...
            <div className="flex flex-wrap gap-4 mt-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-1.5"><Building2 className="h-4 w-4" /> {rfp.client?.full_name}</div>
              <div className="flex items-center gap-1.5"><MapPin className="h-4 w-4" /> {rfp.destination}</div>
              <div className="flex items-center gap-1.5"><Calendar className="h-4 w-4" /> {fmtDate(rfp.departure_date)}</div>
            </div>
          </div>
          ...
          <div className="p-8 space-y-6">
            <div>
              ...
              <p className="text-muted-foreground">{rfp.requester_name} ({rfp.requester_email})</p>
            </div>
            ...
            <div>
              ...
              <p className="text-2xl font-bold tracking-tight text-foreground">
                {rfp.budget ? rfp.budget.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'Sob consulta'}
              </p>
            </div>
              ...
              <div className="mt-8 p-6 bg-success/10 border border-success/20 rounded-xl flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-success-foreground/80 mt-1 text-sm">
                    Aprovado por {rfp.approved_by} em {rfp.approved_at ? new Date(rfp.approved_at).toLocaleString('pt-BR') : ''}. A agência já foi notificada para prosseguir com as emissões.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="mt-8 p-6 bg-danger/10 border border-danger/20 rounded-xl flex items-start gap-4">
                ...
                <div>
                  ...
                  {rfp.rejection_reason && (
                    <p className="text-danger-foreground/80 mt-1 text-sm">Motivo: {rfp.rejection_reason}</p>
                  )}
                </div>
              </div>
            )}
