...
import { Bold, Italic, Strikethrough, List, ListOrdered, Quote, Heading2, Heading3, Link as LinkIcon, Image as ImageIcon, Undo, Redo } from 'lucide-react'
...
export function RichTextEditor({ value, onChange }: { value: string, onChange: (v: string) => void }) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm dark:prose-invert max-w-none min-h-[300px] p-4 outline-none focus:ring-0'
      }
    }
  })
  ...
  const btnClass = "p-1.5 rounded hover:bg-surface-alt text-muted-foreground hover:text-foreground disabled:opacity-50"
