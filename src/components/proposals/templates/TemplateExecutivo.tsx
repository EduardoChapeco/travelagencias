  ...
  return (
    <div className="flex flex-col w-full font-sans bg-white text-slate-900 px-16 py-12">
      ...
      <div className="flex justify-between items-start mb-12 break-inside-avoid">
        {vm.agency.logo_url ? (
          <img src={vm.agency.logo_url} crossOrigin="anonymous" alt="Logo" className="h-12 object-contain" />
        ) : (
          <div className="text-2xl font-bold tracking-tight text-slate-900">{vm.agency.name}</div>
        )}
        <div className="text-right">
          <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Proposta Comercial</div>
      ...
      <div className="border border-slate-200 rounded-xl p-8 mb-16 break-inside-avoid">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Aos cuidados de</div>
            <div className="font-semibold text-slate-900">{(p as any).client_name || "Cliente"}</div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Destino</div>
            <div className="font-semibold text-slate-900">{p.destination || "—"}</div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Período</div>
            <div className="font-semibold text-slate-900">
              {p.travel_start ? fmtDate(p.travel_start) : "—"} a {p.travel_end ? fmtDate(p.travel_end) : "—"}
            </div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Passageiros</div>
            <div className="font-semibold text-slate-900">{vm.totalPax ? `${vm.totalPax} pax` : "—"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-10">
              ...
              {p.flights!.map((f, i) => (
                <div key={i} className="border border-slate-200 rounded-xl p-5 break-inside-avoid">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-500">{f.airline || "Cia Aérea"}</span>
                    <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded">{f.flight_number}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-slate-900">{(f.origin || "---").slice(0,3).toUpperCase()}</div>
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-slate-900">{(f.destination || "---").slice(0,3).toUpperCase()}</div>
        ...
        {vm.hasHotels && (
          <div className="break-inside-avoid">
            ...
            <div className="space-y-4">
              {p.hotels!.map((h, i) => (
                <div key={i} className="border border-slate-200 rounded-xl p-5 break-inside-avoid flex gap-6">
                  {h.images?.[0] && (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-6">
                      <div>
                        ...
                        <div className="text-sm font-semibold text-slate-700">{fmtDate(h.checkin)}</div>
                      </div>
                      <div>
                        <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Check-out</div>
                        <div className="text-sm font-semibold text-slate-700">{fmtDate(h.checkout)}</div>
                      </div>
                    </div>
                    {h.rooms?.[0] && (
                      <div className="mt-4 pt-4 border-t border-slate-100 text-sm text-slate-600">
                        <strong>Acomodação:</strong> {h.rooms.map((r: any) => `${r.qty}x ${r.type}`).join(", ")}
                      </div>
                    )}
                  </div>
                </div>
        ...
        {(vm.hasTransfers || vm.hasTours) && (
          <div className="break-inside-avoid">
            ...
            <div className="grid grid-cols-1 gap-3">
              {p.transfers!.map((t, i) => (
                <div key={i} className="border border-slate-200 rounded-lg p-4 flex items-center gap-4 break-inside-avoid">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center shrink-0"><Car className="w-4 h-4 text-slate-500"/></div>
                  <div>
                    <div className="font-semibold text-slate-900">{t.description}</div>
                    <div className="text-xs text-slate-500 mt-1">{t.type === 'private' ? 'Privativo' : 'Regular'} · {t.vehicle} {t.date ? `· ${fmtDate(t.date)}` : ''}</div>
                  </div>
                </div>
              ))}
              {p.tours!.map((t, i) => (
                <div key={i} className="border border-slate-200 rounded-lg p-4 flex items-center gap-4 break-inside-avoid">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center shrink-0"><Compass className="w-4 h-4 text-slate-500"/></div>
                  <div>
                    <div className="font-semibold text-slate-900">{t.description}</div>
        ...
        <div className="mt-16 break-inside-avoid">
          <div className="bg-[#1E293B] rounded-2xl p-10 text-white border border-slate-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-12">
              <div className="flex-1">
                <div className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-2">Valor da Proposta</div>
                <div className="text-6xl mb-6" style={{ fontFamily: "'Bebas Neue', cursive", color: brand === "#0f172a" ? "#38bdf8" : brand }}>
              ...
              <div className="w-full md:w-[45%] space-y-4">
                <div className="bg-white/10 border border-white/20 rounded-xl p-5">
                  <div className="text-[10px] uppercase font-bold tracking-widest text-slate-400 mb-2">Cartão de Crédito</div>
                  <div className="text-lg font-semibold text-white">
                    {vm.totals.parcelasCartao}x de {money(vm.totals.valorParcelaCartao, p.currency)}
                  </div>
                </div>
                
                <div className="bg-white rounded-xl p-5 text-slate-900 border-2" style={{ borderColor: brand === "#0f172a" ? "#38bdf8" : brand }}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="text-[10px] uppercase font-bold tracking-widest text-slate-500">À vista (PIX)</div>
                    <div className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[10px] font-bold">-{vm.totals.descontoPixPercentual}%</div>
                  </div>
      ...
      <div className="mt-auto pt-12 border-t border-slate-200 flex justify-between items-center break-inside-avoid">
        {p.agent_name && (
          <div className="flex items-center gap-4">
            {p.agent_photo_url ? (
              <img src={p.agent_photo_url} crossOrigin="anonymous" className="w-12 h-12 rounded-full object-cover grayscale" />
            ) : (
            ...
            <div>
              ...
              {p.agent_whatsapp && <div className="text-xs text-slate-600 mt-1">{p.agent_whatsapp}</div>}
            </div>
          </div>
        )}
        {vm.agency.logo_url && (
          <img src={vm.agency.logo_url} crossOrigin="anonymous" className="h-8 object-contain grayscale opacity-60" />
        )}
      </div>
