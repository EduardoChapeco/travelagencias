...
export function Column({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex h-full w-[310px] shrink-0 flex-col rounded-md border bg-surface/50 transition-all duration-300 ${
    ...
    >
      ...
      <SortableContext items={cards.map((c) => c.id)} strategy={verticalListSortingStrategy}>
        <div className="flex-1 space-y-3 overflow-y-auto p-3 no-scrollbar cursor-default bg-background/10">
          ...
          {cards.length === 0 && (
            <div className="flex h-24 items-center justify-center rounded-sm border border-dashed border-border/60 bg-surface-alt/10 text-[10px] font-bold uppercase tracking-widest text-muted-foreground transition-colors hover:border-brand/30">
              {stage.id === "pending"
