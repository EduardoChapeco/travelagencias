...
import { 
  Star, Quote, Play, Plane, Mail, Phone, MapPin, 
  Clock, ArrowRight, ChevronLeft, ChevronRight, Send, Check 
} from "lucide-react";
...
export function NewSectionsRenderer({
  ...
  const animation = block.animation || { enabled: false, type: 'none', delay: 0, duration: 600, trigger: 'onEnter' };
  ...
  switch (block.type) {
        ...
        >
          ...
          <div className="absolute inset-0 z-0 overflow-hidden">
            <img 
              ...
              src={config.image || "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1920&q=80"} 
              alt="Hero Background" 
        ...
        <section 
          ...
          style={{
            background: `linear-gradient(${
              config.gradientDirection === 'to-b' ? '180deg' : config.gradientDirection === 'to-br' ? '135deg' : '90deg'
        ...
        >
          {config.floatingIcons && (
            <div className="absolute inset-0 z-0 pointer-events-none opacity-20">
              <div className="absolute top-[20%] left-[10%] animate-pulse"><Plane className="h-8 w-8 text-white" /></div>
              <div className="absolute bottom-[30%] right-[15%] animate-bounce duration-[4s]"><Star className="h-6 w-6 text-white fill-current" /></div>
            </div>
          )}
      ...
      return (
        ...
        >
          <div className="absolute inset-0 z-0">
            <img 
              src={config.backgroundImage || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?auto=format&fit=crop&w=1920&q=80"} 
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
                ...
                <div className="flex gap-8 pt-6 border-t border-border">
                  {config.metrics.map((m: any, i: number) => (
                    <div key={i} className="space-y-1">
                      ...
                      <div className="text-xs font-semibold text-muted-foreground uppercase">{m.label}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="w-full lg:w-1/2">
              <div className="grid grid-cols-2 gap-4">
                {config.bentoCells && Array.isArray(config.bentoCells) && config.bentoCells.slice(0, 4).map((cell: any, i: number) => {
                  if (cell.type === 'stat') {
                    return (
                      <div 
                        key={i} 
                      ...
                      >
                        ...
                        <div className="text-xs font-semibold text-muted-foreground uppercase leading-relaxed">{cell.label}</div>
                      </div>
                    );
                  }
                  return (
      ...
      return (
        ...
        >
          ...
          >
            {config.headline} <br />
            {config.accentWord && (
              <span style={{ color: "var(--brand-primary, #1E3A5F)" }}>
                {config.accentWord}
              </span>
            )}
          </h1>
      ...
      return (
        ...
        >
          <div className="text-center space-y-3 mb-12">
            {config.sectionLabel && (
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "var(--brand-primary, #1E3A5F)" }}>
                {config.sectionLabel}
              </span>
            )}
            <h2 style={headingStyle} className="text-3xl md:text-4xl font-extrabold text-foreground">
              {config.sectionTitle}
            </h2>
            ...
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 style={headingStyle} className="text-xl font-bold text-foreground mb-2">{item.name}</h3>
                    <div className="flex items-center gap-1 mb-4">
                      {config.showRatings && Array.from({ length: item.rating || 5 }).map((_, si) => (
          ...
          >
            {items.map((item: any, i: number) => (
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 style={headingStyle} className="text-lg font-bold text-foreground mb-1">{item.name}</h3>
    ...
    case "destinations-slider-film": {
      ...
      return (
        <section 
          ...
          style={{ background: `linear-gradient(135deg, ${config.color1 || '#FFEBE7'}, ${config.color2 || '#FFFFFF'})` }}
        >
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">
            <div className="w-full lg:w-2/5 space-y-6">
              ...
              <h2 style={headingStyle} className="text-3xl md:text-5xl font-black text-foreground leading-tight">
                {config.headline}
              </h2>
              ...
              {config.ctaText && (
                <div className="pt-2">
                  <button className="px-6 py-3 rounded-xl text-sm font-bold text-white" style={{ backgroundColor: "var(--brand-primary, #1E3A5F)" }}>
                    {config.ctaText}
                  </button>
                </div>
              )}
    ...
    case "package-cards": {
      const showCols = config.columns === '4' ? 'lg:grid-cols-4' : config.columns === '2' ? 'lg:grid-cols-2' : 'lg:grid-cols-3';
      return (
        <section 
          ref={animRef as any}
          className="py-16 px-4 max-w-7xl mx-auto w-full"
            ...
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              ...
              >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div className="space-y-4">
                    <h3 style={headingStyle} className="text-xl font-bold text-foreground leading-tight group-hover:text-brand transition-colors">
                      {item.title}
                    </h3>
                  ...
                  <div className="pt-4 flex items-end justify-between">
                    <div>
                      {config.showOriginalPrice && item.priceOriginal && (
                        <div className="text-xs text-muted-foreground line-through">{item.priceOriginal}</div>
                      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 relative aspect-[16/10] md:aspect-auto rounded-3xl overflow-hidden group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-8 text-white space-y-2">
                ...
                <h3 style={headingStyle} className="text-3xl font-black">{config.promoDestination}</h3>
            ...
            <div className="bg-surface-alt/40 border border-border/60 p-8 rounded-3xl flex flex-col justify-center space-y-4">
              <h4 style={headingStyle} className="text-xl font-bold text-foreground">{config.newsTitle}</h4>
              <p className="text-muted-foreground text-xs leading-relaxed">Assine nossa lista de transmissão rápida e receba alertas de tarifas promocionais.</p>
    ...
    case "features-grid": {
      const showCols = config.columns === '4' ? 'lg:grid-cols-4' : 'lg:grid-cols-3';
      const cardStyles = 
        config.cardStyle === 'tinted' ? 'bg-brand/5 border-transparent shadow-none' : 
        config.cardStyle === 'minimal' ? 'bg-transparent border-transparent shadow-none p-4' : 
        'bg-surface border-border/80 shadow';

      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-7xl mx-auto w-full">
          <div className="text-center space-y-3 mb-16">
            <h2 style={headingStyle} className="text-3xl md:text-4xl font-extrabold text-foreground">{config.sectionTitle}</h2>
            {config.sectionDescription && (
              <p style={bodyStyle} className="text-muted-foreground max-w-xl mx-auto">{config.sectionDescription}</p>
            )}
          </div>
            ...
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              ...
              >
                <div 
                  ...
                  style={{ backgroundColor: "var(--brand-light, #F1F5F9)", color: "var(--brand-primary, #1E3A5F)" }}
                >
                  {renderIconByName(item.icon, "h-6 w-6")}
                </div>
                <h3 style={headingStyle} className="text-lg font-bold text-foreground mb-3">{item.title}</h3>
                <p style={bodyStyle} className="text-muted-foreground text-xs leading-relaxed">{item.description}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-7xl mx-auto w-full">
          <div className="mb-12 text-center">
            <h2 style={headingStyle} className="text-3xl font-extrabold text-foreground">{config.sectionTitle}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => {
              const span = i === 0 ? 'md:col-span-2' : '';
              const bg = i === 0 ? 'bg-brand text-white border-transparent' : 'bg-surface border-border';
              ...
              return (
                ...
                >
                  <div 
                    className="flex h-12 w-12 items-center justify-center rounded-2xl mb-6 bg-brand/10 text-brand"
                    style={i === 0 ? { backgroundColor: "rgba(255,255,255,0.15)", color: "#fff" } : { backgroundColor: "var(--brand-light, #F1F5F9)", color: "var(--brand-primary, #1E3A5F)" }}
                  >
                    {renderIconByName(item.icon, "h-6 w-6")}
                  </div>
                  <div>
                ...
                <div key={i} className={`flex flex-col${flexDir}items-center gap-8 md:gap-16`}>
                  <div className="w-full md:w-1/2 space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 style={headingStyle} className="text-xl font-bold text-foreground">{item.title}</h3>
                    </div>
    ...
    case "stats-strip": {
      const bgStyle = 
        config.backgroundStyle === 'dark' ? 'bg-[#0A0A0A] text-white border-transparent' : 
        config.backgroundStyle === 'brand' ? 'bg-brand text-brand-foreground border-transparent' : 
        'bg-surface border-border';
      
      const textColor = 
        config.backgroundStyle === 'dark' ? 'text-white' : 
        config.backgroundStyle === 'brand' ? 'text-brand-foreground' : 
        'text-foreground';

      const labelColor = 
        config.backgroundStyle === 'dark' ? 'text-neutral-400' : 
        config.backgroundStyle === 'brand' ? 'text-brand-foreground/80' : 
    ...
    case "brands-logos": {
      ...
      const colClass = config.gridColumns === '6' ? 'grid-cols-3 md:grid-cols-6' : 'grid-cols-2 md:grid-cols-4';
      return (
        <section ref={animRef as any} className="py-12 border-y border-border/40 w-full overflow-hidden">
          {config.sectionTitle && (
            <h3 style={headingStyle} className="text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-8">
              {config.sectionTitle}
            </h3>
          )}
          ...
          {isMarquee ? (
            <div className="flex overflow-hidden relative w-full pointer-events-none select-none">
              <div className="flex gap-16 animate-[marquee_25s_linear_infinite] shrink-0 min-w-full justify-around items-center">
                {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
                  <img key={i} src={item.logoUrl} alt={item.altText} className="h-8 md:h-10 object-contain opacity-40 hover:opacity-80 transition-opacity" />
                ))}
              </div>
              {/* Duplicate array for continuous infinite scroll */}
              <div className="flex gap-16 animate-[marquee_25s_linear_infinite] shrink-0 min-w-full justify-around items-center" aria-hidden="true">
                {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
                  <img key={i} src={item.logoUrl} alt={item.altText} className="h-8 md:h-10 object-contain opacity-40 hover:opacity-80 transition-opacity" />
                ))}
              </div>
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-7xl mx-auto w-full">
          <div className={`flex flex-col lg:flex-row items-center gap-12${config.layout === 'text-left' ? 'lg:flex-row-reverse' : ''}`}>
            ...
            <div className="w-full lg:w-1/2 space-y-6">
              {config.sectionLabel && (
                <span className="text-xs font-bold uppercase tracking-widest text-brand" style={{ color: "var(--brand-primary, #1E3A5F)" }}>
                  {config.sectionLabel}
                </span>
              )}
              <h2 style={headingStyle} className="text-3xl md:text-4xl font-extrabold text-foreground leading-snug">
                {config.headline}
              </h2>
              <div style={bodyStyle} className="space-y-4 text-muted-foreground text-sm leading-relaxed">
              ...
              {config.bullets && Array.isArray(config.bullets) && (
                <ul className="space-y-3 pt-2">
                  {config.bullets.map((item: any, i: number) => (
                    <li key={i} className="flex items-center gap-2.5 text-xs font-semibold text-foreground">
    ...
    case "service-cards": {
      ...
      const bgClass = 
        isDark ? 'bg-[#111111] text-white border-transparent' : 
        isBrand ? 'bg-brand text-brand-foreground border-transparent' : 
        'bg-surface border-border';
      
      const textClass = 
        isDark ? 'text-white' : 
        isBrand ? 'text-brand-foreground' : 
        'text-foreground';

      const descClass = 
        isDark ? 'text-neutral-400' : 
        isBrand ? 'text-brand-foreground/80' : 
        'text-muted-foreground';
            ...
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              ...
              >
                <div 
                  ...
                  style={isBrand ? { backgroundColor: 'rgba(255,255,255,0.15)', color: '#fff' } : { backgroundColor: "var(--brand-light, #F1F5F9)", color: "var(--brand-primary, #1E3A5F)" }}
                >
                  {renderIconByName(item.icon, "h-6 w-6")}
                </div>
              ...
              <div key={i} className="group bg-surface border border-border/60 rounded-2xl overflow-hidden transition-all flex flex-col justify-between">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div className="space-y-3">
                    ...
                    <h3 style={headingStyle} className="text-base font-bold text-foreground leading-snug group-hover:text-brand transition-colors">{post.title}</h3>
                    <p style={bodyStyle} className="text-xs text-muted-foreground line-clamp-3 leading-relaxed">{post.snippet}</p>
                  </div>
            ...
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              <div key={i} className="bg-surface border border-border/60 p-6 rounded-3xl text-center space-y-4 transition-shadow">
                ...
                <div className="space-y-1">
                  <h3 style={headingStyle} className="text-lg font-bold text-foreground">{item.name}</h3>
                  <div className="text-xs font-semibold text-brand" style={{ color: "var(--brand-primary, #1E3A5F)" }}>{item.role}</div>
                </div>
                <p style={bodyStyle} className="text-xs text-muted-foreground leading-relaxed px-4">{item.bio}</p>
                {item.whatsapp && (
                  <div className="pt-2">
                    <a 
          ...
          <div className="relative border-l-2 border-brand/20 ml-4 md:ml-32 space-y-12 pb-4">
            {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
              <div key={i} className="relative pl-6 md:pl-8">
                {/* Time bubble */}
                <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full border-2 border-white bg-brand" style={{ backgroundColor: "var(--brand-primary, #1E3A5F)" }} />
                
                {/* Left side label on wide screens */}
                <div className="absolute right-full mr-8 top-1.5 hidden md:block text-right">
                  <span className="text-sm font-black text-brand" style={{ color: "var(--brand-primary, #1E3A5F)" }}>{item.time}</span>
                </div>

                <div className="space-y-1">
                  <span className="text-xs font-bold text-brand md:hidden block mb-1" style={{ color: "var(--brand-primary, #1E3A5F)" }}>{item.time}</span>
                  <h3 style={headingStyle} className="text-base font-bold text-foreground">{item.title}</h3>
                  <p style={bodyStyle} className="text-xs text-muted-foreground leading-relaxed max-w-xl">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    case "testimonial-single": {
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-3xl mx-auto text-center space-y-6">
          <Quote className="h-10 w-10 mx-auto text-brand/25" style={{ color: "var(--brand-primary, #1E3A5F)", opacity: 0.25 }} />
          <p style={headingStyle} className="text-xl md:text-2xl font-semibold italic text-foreground leading-relaxed">
            "{config.text}"
          </p>
    ...
    case "testimonial-carousel": {
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-4xl mx-auto w-full text-center space-y-8">
          <h2 style={headingStyle} className="text-3xl font-extrabold text-foreground mb-8">{config.sectionTitle}</h2>
          
          <div className="space-y-6">
            <p style={headingStyle} className="text-lg md:text-xl font-medium italic text-foreground max-w-2xl mx-auto leading-relaxed">
              "{activeItem.text}"
            </p>
            ...
            <div>
              ...
              {activeItem.role && <div className="text-xs text-muted-foreground">{activeItem.role}</div>}
            </div>
          </div>
              ...
              <div key={i} className="bg-surface border border-border p-6 rounded-3xl flex flex-col justify-between transition-shadow">
                ...
                <div className="flex items-center gap-3 border-t border-border pt-4">
                  {item.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-brand/10 text-brand flex items-center justify-center font-bold text-sm" style={{ backgroundColor: "var(--brand-light, #F1F5F9)", color: "var(--brand-primary, #1E3A5F)" }}>
    ...
    case "testimonial-dark": {
      return (
        <section ref={animRef as any} className="py-16 px-4 w-full text-white" style={{ backgroundColor: '#0D0D0D' }}>
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center">
              <h2 style={headingStyle} className="text-3xl font-extrabold">{config.sectionTitle}</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {config.items && Array.isArray(config.items) && config.items.map((item: any, i: number) => (
                <div key={i} className="bg-[#1A1A1A] p-6 rounded-3xl border border-neutral-800 space-y-4">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-1">
                      {Array.from({ length: item.stars || 5 }).map((_, si) => (
                        <Star key={si} className="h-4.5 w-4.5 fill-current text-brand" style={{ color: "var(--brand-secondary, #D4AF37)" }} />
                      ))}
                    </div>
                  </div>
                  <p style={bodyStyle} className="text-sm text-neutral-300 italic">"{item.text}"</p>
        ...
        <section ref={animRef as any} className="py-16 px-4 max-w-6xl mx-auto w-full">
          <div className="bg-surface-alt/45 p-8 md:p-12 rounded-3xl border border-border flex flex-col md:flex-row gap-8 md:gap-12 items-center">
            ...
            <div className="w-full md:w-3/5 space-y-4">
              ...
              <p style={headingStyle} className="text-lg md:text-xl font-medium italic text-foreground leading-relaxed">
                "{config.text}"
              </p>
    ...
    case "gallery-grid": {
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-7xl mx-auto w-full space-y-8">
          {config.sectionTitle && (
            <h2 style={headingStyle} className="text-2xl font-extrabold text-center text-foreground">{config.sectionTitle}</h2>
          )}
    ...
    case "video-section": {
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-4xl mx-auto w-full space-y-6 text-center">
          {config.sectionTitle && (
    ...
    case "photo-text-alternating": {
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-5xl mx-auto w-full space-y-16">
          {config.sectionTitle && (
            <h2 style={headingStyle} className="text-3xl font-extrabold text-center text-foreground">{config.sectionTitle}</h2>
          )}
      ...
      return (
        ...
        >
          ...
          <div className="relative z-10 max-w-3xl w-full text-white space-y-6">
            {config.badgeText && (
              <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-white/10 border border-white/20 uppercase tracking-widest">{config.badgeText}</span>
            )}
            <h2 style={headingStyle} className="text-3xl md:text-5xl font-black">{config.headline}</h2>
            <p style={bodyStyle} className="text-base md:text-lg text-white/80 leading-relaxed max-w-xl mx-auto">{config.subtitle}</p>
            {config.ctaText && config.ctaUrl && (
              <div className="pt-2">
                <a 
                  href={config.ctaUrl}
    ...
    case "cta-banner": {
      ...
      const bgStyle = 
        isBrand ? 'bg-brand text-brand-foreground border-transparent' :
        isGrad ? 'bg-gradient-to-r from-brand to-brand-light text-brand-foreground border-transparent' :
      ...
      return (
        ...
        >
          ...
          {config.subtitle && (
            <p style={bodyStyle} className="text-sm md:text-base opacity-85 max-w-xl mx-auto leading-relaxed">{config.subtitle}</p>
          )}
          <div className="pt-2">
            <a 
              href={config.ctaUrl}
    ...
    case "cta-newsletter": {
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-4xl mx-auto w-full text-center space-y-6">
          <div className="space-y-2">
            <h2 style={headingStyle} className="text-2xl md:text-3xl font-extrabold text-foreground">{config.title}</h2>
            {config.subtitle && <p style={bodyStyle} className="text-muted-foreground text-sm max-w-md mx-auto">{config.subtitle}</p>}
          </div>
    ...
    case "cta-contact-form": {
      ...
      const handleSubmit = async () => {
        if (!form.name.trim() || !form.email.trim() || !form.message.trim()) {
          return toast.error("Por favor, preencha os campos obrigatórios (Nome, E-mail e Mensagem).");
        }
        setSending(true);
      ...
      return (
        <section ref={animRef as any} className="py-16 px-4 max-w-3xl mx-auto w-full space-y-8">
          <div className="text-center space-y-3">
            <h2 style={headingStyle} className="text-3xl font-extrabold text-foreground">{config.title}</h2>
            {config.subtitle && <p style={bodyStyle} className="text-muted-foreground text-sm max-w-xl mx-auto">{config.subtitle}</p>}
          </div>
          ...
          {success ? (
            <div className="bg-green-500/10 border border-green-500/30 p-8 rounded-3xl text-center space-y-4">
              ...
              <h3 style={headingStyle} className="text-lg font-bold text-green-600">Enviado com Sucesso!</h3>
              <p style={bodyStyle} className="text-xs text-muted-foreground max-w-sm mx-auto">{config.successMessage}</p>
            </div>
          ) : (
            <div className="bg-surface border border-border p-8 rounded-3xl space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-muted-foreground">Destino de Interesse</label>
                  <input 
                    type="text" 
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-muted-foreground">Mensagem / Preferências de Viagem *</label>
                <textarea 
                  rows={4}
      ...
      return (
        <nav className={`w-full py-4 px-6 flex items-center justify-between${bgStyle}`}>
          ...
          <div className="flex items-center gap-3 select-none">
            ...
            <span style={headingStyle} className="font-extrabold tracking-tight text-sm">TravelOS</span>
          </div>
    ...
    case "footer-minimal": {
      return (
        <footer ref={animRef as any} className="py-8 border-t border-border/60 text-center text-xs text-muted-foreground space-y-4">
          <div>{config.copyright || `© ${new Date().getFullYear()} Agência. Todos os direitos reservados.`}</div>
        </footer>
      );
    }
    ...
    case "footer-dark": {
      ...
      const bg = isDark ? 'bg-[#0A0A0A] text-white border-transparent' : 'bg-surface border-t border-border/40';
    ...
    case "footer-cta": {
      return (
        <footer ref={animRef as any} className="bg-surface border-t border-border/40 py-12 px-6">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="p-8 rounded-3xl text-white text-center space-y-4" style={{ backgroundColor: "var(--brand-primary, #1E3A5F)" }}>
              <h3 style={headingStyle} className="text-xl md:text-2xl font-black">{config.ctaTitle}</h3>
              <div className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto pt-2">
                <input type="email" placeholder="Seu e-mail" className="flex-1 p-2.5 rounded-xl border-none outline-none bg-white text-black text-xs" />
                <button 
        ...
        <footer ref={animRef as any} className="py-12 px-6 bg-surface border-t border-border/40">
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-center">
            ...
            <div className="w-full lg:w-1/2 space-y-4 text-center lg:text-left">
              <h3 style={headingStyle} className="font-extrabold text-lg text-foreground">Sede da Agência</h3>
              <p style={bodyStyle} className="text-xs text-muted-foreground leading-relaxed">{config.description}</p>
            </div>
          </div>
        </footer>
      );
