  ...
  return (
    <div className="bg-[var(--surface)] rounded-2xl border p-6 flex flex-col h-full min-h-[600px]">
      ...
      <div className="grid grid-cols-7 gap-2 flex-1 min-h-0">
        ...
        {prefixDays.map((_, i) => (
          <div key={`prefix-${i}`} className="p-2 rounded-xl bg-[var(--surface-alt)]/20 opacity-30 border border-transparent" />
        ))}
        ...
        {daysInMonth.map((day) => {
          ...
          return (
            <div 
              key={day.toString()} 
              className={`p-2 rounded-xl border flex flex-col min-h-[100px] transition-all bg-[var(--surface)] ${
                isToday 
