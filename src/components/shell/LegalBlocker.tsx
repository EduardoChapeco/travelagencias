...
export function LegalBlocker({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center bg-background"><div className="animate-pulse text-sm">Verificando conformidade legal...</div></div>;
  }
        ...
        <div className="flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          <div className="border-b border-border bg-surface-alt/50 px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold tracking-tight">Atualização dos Termos de Uso (LGPD)</h2>
                <p className="text-xs text-muted-foreground">Versão: {data.doc.version} · Efetivo a partir de {new Date(data.doc.effective_at).toLocaleDateString("pt-BR")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-border bg-surface-alt/30 p-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-xs text-muted-foreground flex-1">
                Ao clicar em "Eu Aceito", você concorda com a Política de Privacidade e os Termos de Uso do sistema, 
                garantindo conformidade com a LGPD. Seu aceite será registrado com trilha de auditoria inviolável.
              </p>
              <button
                onClick={() => acceptMutation.mutate(data.doc.id)}
                disabled={acceptMutation.isPending || (!scrolledToBottom && data.doc.content_md.length > 500)}
