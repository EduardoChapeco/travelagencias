...
export function CommandMenu() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "client": return <User className="h-4 w-4 mr-2 text-blue-500" />;
      case "lead": return <Briefcase className="h-4 w-4 mr-2 text-orange-500" />;
      case "trip": return <Compass className="h-4 w-4 mr-2 text-green-500" />;
      case "ticket": return <Ticket className="h-4 w-4 mr-2 text-red-500" />;
      default: return <FileText className="h-4 w-4 mr-2 text-muted-foreground" />;
    }
  };
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "lead": return "Oportunidade";
      case "trip": return "Viagem";
      case "ticket": return "Suporte";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] bg-background/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-2xl bg-surface border border-border shadow-2xl rounded-xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <Command label="Global Search" shouldFilter={false} className="flex flex-col w-full bg-transparent">
          <div className="flex items-center border-b border-border px-4 py-3">
            ...
            <Command.Input
              ...
              autoFocus
            />
            {loading && <div className="h-4 w-4 border-2 border-brand border-t-transparent rounded-full animate-spin ml-2"></div>}
            <button onClick={() => setOpen(false)} className="ml-2 text-[10px] font-medium px-1.5 py-0.5 bg-surface-alt rounded text-muted-foreground border border-border hover:bg-border transition-colors">ESC</button>
          </div>
