              ...
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Nome do Solicitante (Passageiro) *">
                    <Input value={requesterName} onChange={(e) => setRequesterName(e.target.value)} placeholder="Ex: João Silva" />
                  </Field>
                  <Field label="E-mail Corporativo do Solicitante *">
                    <Input type="email" value={requesterEmail} onChange={(e) => setRequesterEmail(e.target.value)} placeholder="joao.silva@empresa.com" />
                  </Field>
                </div>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <Field label="Título da Requisição *">
                  <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Reunião Anual SP 2026" className="font-semibold text-lg" autoFocus />
                </Field>
                <Field label="Destino da Viagem *">
                  <Input value={destination} onChange={(e) => setDestination(e.target.value)} placeholder="Ex: São Paulo, SP" />
                </Field>
                <div className="grid grid-cols-2 gap-4">
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <Field label="Orçamento Máximo Aprovado (R$ / Opcional)">
                  <Input type="number" step="0.01" min="0" value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="0.00" className="text-lg font-mono text-brand" />
                  <p className="text-[10px] text-muted-foreground mt-1">Ao definir o orçamento, o sistema alertará se a cotação violar a política da empresa.</p>
                </Field>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="rounded-xl border border-border bg-surface-alt/20 p-6">
                  ...
                  <div className="grid grid-cols-2 gap-y-4 text-sm mt-4 border-t border-border pt-4">
                    <div>
                      <div className="text-xs text-muted-foreground mb-1 uppercase tracking-widest font-semibold flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5"/> Empresa</div>
                      <div className="font-medium">{selectedClient?.full_name || "—"}</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground mb-1 uppercase tracking-widest font-semibold">Solicitante</div>
                      <div className="font-medium">{requesterName} <span className="text-xs text-muted-foreground">({requesterEmail})</span></div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground mb-1 uppercase tracking-widest font-semibold flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5"/> Destino</div>
                      <div className="font-medium">{destination}</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground mb-1 uppercase tracking-widest font-semibold flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5"/> Datas</div>
                      <div className="font-medium">{departureDate ? new Date(departureDate).toLocaleDateString('pt-BR') : '—'} {returnDate ? `até ${new Date(returnDate).toLocaleDateString('pt-BR')}` : ''}</div>
                    </div>
                  </div>

                  {budget && (
                    <div className="mt-4 p-4 bg-brand/5 border border-brand/20 rounded-lg flex justify-between items-center">
